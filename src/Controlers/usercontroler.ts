...
try {
    ...
    const user=(await DatabaseHelper.query('SELECT  * FROM Users WHERE email=${email}')).recordset[0]
    if(user){
...
try {
    ...
    let user:iusers[]= await( await DatabaseHelper.exec('authenticateUsers',{password,email})).recordset[0]
    ...
    return res.status(201).json("Logged in Successful")
